...
def test_classy_sz_clgyappag():
   ...
   #  print(gy_2h)
    ...
    for i, l in enumerate(ell_list[:14]):
        f = l*(l+1)/2/np.pi
        diff_1h = gy_1h[i]-f*gy_1h_shivam[i]
        diff_2h = gy_2h[i]-f*gy_2h_shivam[i]
        ...
        assert f*gy_2h_shivam[i]  == pytest.approx(gy_2h[i], rel=5e-2), f" Relative error in '2h' term of g x y at ell={l} is {diff_2h/gy_2h[i]}, expected  < 7e-2"
