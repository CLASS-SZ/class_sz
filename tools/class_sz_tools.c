 ...
 // printf("z = %.5e ng = %.5e\n",z,V->pvectsz[V->ptsz->index_mean_galaxy_number_density]);
  ...
  if ((V->ptsz->has_gal_gal_hf == _TRUE_) && (index_md == V->ptsz->index_md_gal_gal_hf)) {
    ...
    if (V->ptsz->use_nl_bias){
      ...
      double pknl = get_pk_nonlin_at_k_and_z((l+0.5)/chi,z,V->pba,V->ppm,V->pfo,V->ptsz);
      double pkl = get_pk_nonlin_at_k_and_z((l+0.5)/chi,z,V->pba,V->ppm,V->pfo,V->ptsz);
...
// #pragma omp flush(abort)
  ...
  if (ptsz->need_sigma == 1){
      ...
      // printf("s=%.6e r=%.6e z=%.6e sigma_var = %.3e\n",
