...
// printf("entering szp module");
  ...
  // printf("tabulating dndlnM quantities %d\n",ptsz->has_sigma2_hsv);
   ...
   // printf("tabulating dndlnM quantities 2\n");
...
double cl_kSZ2_gal_fft = 0.;
// omp_set_lock(lock); //Only a single thread writes
// printf("My Thread num is: %d\n", id);

int index_ell = (int) Pvectsz[ptsz->index_multipole];
// if (0==0){

int index_z;
double z_min = 1.0001*ptsz->z1SZ;
double z_max = 0.9999*ptsz->z2SZ;

int index_m;
double m_min = 1.0001*ptsz->M1SZ;
double m_max = 0.9999*ptsz->M2SZ;
...
int n_m_fft = 300;
// double cl_kSZ2_gal_fft = 0.;
double dln1pz  = (log(1.+z_max)-log(1.+z_min))
                  /(n_z_fft-1.);
double dlnm = (log(m_max)-log(m_min))/(n_m_fft-1.);

// printf("starting fft computation loop z\n");

for (index_z=0; index_z<n_z_fft; index_z++)
{
/// chose z and m :
// double z = 0.5;

double z  =  exp(log(1.+z_min)
              +index_z*(log(1.+z_max)-log(1.+z_min))
              /(ptsz->n_arraySZ-1.))-1.; // log(1+z)
// if (index_z == ptsz->n_arraySZ-1){
//   z = 0.999999*z;
// }
...
double phi_galaxy_at_z = get_galaxy_number_counts(z,ptsz); // this is ptsz->normalized_dndz_phig
double Wg = H_over_c_in_h_over_Mpc*phi_galaxy_at_z;
double vrms2 = get_vrms2_at_z(z,ptsz);

double cl_kSZ2_gal_fft_z  = 0.;
for (index_m=0; index_m<n_m_fft; index_m++)
{
double m = exp(log(m_min)+index_m*(log(m_max)-log(m_min))/(ptsz->ndimSZ-1.));
...
taul = get_tau_profile_at_z_m_l(z,m,l,ptsz,pba);
Pk[ik] = fl*taul;
Pko[ik] = fl*taul;
// printf("k = %.5e pk = %.5e\n",l,Pk[ik]);
}
...
lnpk[ik] = log(fabs(Pk[ik]));
   ...
   double g = get_galaxy_profile_at_z_m_l_1h(z,m,ell,ptsz,pba); // this has 1/ng_bar
      // printf("ok3\n");
   // double ell = l3;
   double dn = get_dndlnM_at_z_and_M(z,m,ptsz);
      // printf("ok4\n");
   // for (ik=0; ik<N; ik++){
   // printf("k = %.5e f_tau_f_tau = %.5e g = %.5e dn = %.5e\n",ell,f_tau_f_tau,g,dn);
   // lnpk[ik] = log(Pk[ik]);
   // }
...
cl_kSZ2_gal_fft_z += integrand_projected_fields*dlnm;
// printf("cl_kSZ2_gal_fft_z = %.5e\n",cl_kSZ2_gal_fft_z);

}


cl_kSZ2_gal_fft_z *= Wg/chi/chi;


cl_kSZ2_gal_fft_z *= vrms2;///3./pow(_c_*1e-3,2.);

cl_kSZ2_gal_fft_z *= chi*chi;
cl_kSZ2_gal_fft_z *= (1.+z)/H_over_c_in_h_over_Mpc;

cl_kSZ2_gal_fft += cl_kSZ2_gal_fft_z*dln1pz;
// printf("cl_kSZ2_gal_fft = %.5e dln1pz = %.5e\n",cl_kSZ2_gal_fft,dln1pz);
...
// }
// omp_unset_lock(lock);
printf("cl_kSZ2_gal_fft = %.5e \n",cl_kSZ2_gal_fft);
ptsz->cl_kSZ_kSZ_gal_1h_fft[index_ell] = cl_kSZ2_gal_fft/(2.*_PI_)/(2.*_PI_);
// exit(0);
...
pvectsz[ptsz->index_integrand] = integrand_projected_fields;
  ...
  //  double l,m,z;
  //  z = pvectsz[ptsz->index_z];
  //  m = exp(logM);
  //  // l = l3;
  //  double g3 = get_galaxy_profile_at_z_m_l_1h(z,m,l3,ptsz,pba); // this has 1/ng_bar
  //  double t2 =  get_tau_profile_at_z_m_l(z,m,l2,ptsz,pba);
  //  double t1 =  get_tau_profile_at_z_m_l(z,m,l1,ptsz,pba);
  //  double dn = get_dndlnM_at_z_and_M(z,m,ptsz);
  // pvectsz[ptsz->index_integrand] = dn*g3*t2*t1;

  // printf("g3 %.3e %.3e\n",g3,galaxy_profile_at_ell_3);
  // printf("t2 %.3e %.3e\n",t2,tau_profile_at_ell_2);
  // printf("t1 %.3e %.3e\n",t1,tau_profile_at_ell_1);
  // printf("m %.3e dn %.3e %.3e\n",m,dn,pvectsz[ptsz->index_hmf]);
...
// printf("tsz lensmag 2h %.3f b = %.3f mu = %.3f\n",1.,pvectsz[ptsz->index_halo_bias],pvectsz[ptsz->index_lensing_profile]);
    ...
    pvectsz[ptsz->index_integrand] =  pvectsz[ptsz->index_hmf]
                                      ...
                                      *galaxy_profile_at_k_1
                                      *damping_1h_term;
    // printf("%e %e %e\n",pvectsz[ptsz->index_hmf],galaxy_profile_at_k_1,damping_1h_term);
   }
...
//  if (ptsz->tau_profile == 0){
  ...
  // note that tau_profile has a normalisation proportional to:
  // double tau_normalisation = pvectsz[ptsz->index_m200m]/(4.*_PI_*pow(pvectsz[ptsz->index_rs],3.));
  // so that the 4*pi*rs^3 simplifies
...
double tau_e  = get_density_profile_at_l_M_z(l,m,z,ptsz);
...
return tau_e;
 ...
 // }
 //   result = get_density_profile_at_l_M_z(l_asked,m_asked,z_asked,ptsz);
  // printf("result ft = %.3e interpolated = %.3e\n",result_ft,result);
  //
  ...
  //
  //
  // tau_normalisation = 1.;//pba->Omega0_b/ptsz->Omega_m_0/ptsz->mu_e*ptsz->f_free;
  //
  //
  // double sigmaT_over_mp = 8.305907197761162e-17 * pow(pba->h,2)/pba->h; // !this is sigmaT / m_prot in (Mpc/h)**2/(Msun/h)
  ...
  // // sigma_thomson_in_Mpc2 = sigma_thomson_in_m2*Mpc_over_m**-2
  // // mp_in_kg = 1.6726219e-27
  // // Msun_in_kg = 1.9884e30
  // // mp_in_msun = mp_in_kg*Msun_in_kg**-1
  // // sigmaT_over_mp = sigma_thomson_in_Mpc2/mp_in_msun
  // // print(sigmaT_over_mp)
  // // 8.305907197761162e-17
                             ...
                             struct tszspectrum * ptsz)
{
  ...
  if (ptsz->MF == 1){
   ...
   // printf("ls = %.3e rs = %.3e\n",pvectsz[ptsz->index_rs],pvectsz[ptsz->index_ls]);
  ...
  // printf("getting mu\n");
   ...
   // double l_asked = pvectsz[ptsz->index_multipole_for_nfw_profile];
   // double m_asked = mass_nfw; // in Msun/h
   // double z_asked = pvectsz[ptsz->index_z];
   // result = get_density_profile_at_l_M_z(l_asked,m_asked,z_asked,ptsz);
  // printf("getting mu  = %.3f\n",result);
 // }
                 ...
                 struct tszspectrum * ptsz)
{
   ...
   if (
       pvectsz[ptsz->index_has_electron_pressure]+
       pvectsz[ptsz->index_has_electron_pressure] +
       pvectsz[ptsz->index_has_electron_density] +
       pvectsz[ptsz->index_has_galaxy] +
       pvectsz[ptsz->index_has_matter_density] +
       pvectsz[ptsz->index_has_lensing] +
       pvectsz[ptsz->index_has_cib] +
       pvectsz[ptsz->index_has_isw] == 0){
       pvectsz[ptsz->index_mass_for_hmf] = exp(logM);

       // printf("skipping mass conversion in hmf\n");
   }
   else
   {
 ...
 //
 // printf("m_e = %.3e m_t = %.3e\n",mdel_prime,get_m200m_to_m500c_at_z_and_M(z,pvectsz[ptsz->index_m200m],ptsz));
...
// if (ptsz->has_electron_pressure == 1 && ptsz->pressure_profile == 4){
//    //compute m200c
//   class_call(mVIR_to_mDEL(pvectsz[ptsz->index_mVIR],
//                        pvectsz[ptsz->index_rVIR],
//                        pvectsz[ptsz->index_cVIR],
...
// }
  ...
  //                  }
  //    else{
  //   // !! Careful: uncomment if you want to integrate with repect to m_delt
  ...
  //
   ...
   //     pvectsz[ptsz->index_ls] =  sqrt(pvectsz[ptsz->index_chi2])/(1.+z)/pvectsz[ptsz->index_rs];
    ...
    // pvectsz[ptsz->index_l200c] = sqrt(pvectsz[ptsz->index_chi2])/(1.+z)/pvectsz[ptsz->index_r200c];
    //
    // //pvectsz[ptsz->index_Delta_c]= Delta_c_of_Omega_m(pvecback[pba->index_bg_Omega_m]);
...
// printf("printing to files\n");
            // printf("%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\t\t%e\n",
            //         ptsz->ell[index_l],
            //         ptsz->cl_sz_1h[index_l],
            //         sig_cl_squared,
...
int select_multipole_array(struct tszspectrum * ptsz)
{
  ...
  //                                                  - log(2.))/(ptsz->N_kSZ2_gal_multipole_grid-1.));
   ...
   // ptsz->theta_kSZ2_gal_theta_grid[index_l] = -1 + index_l*2./(ptsz->N_kSZ2_gal_theta_grid-1.);
...
int initialise_and_allocate_memory(struct tszspectrum * ptsz){
 ...
 if (ptsz->has_sz_counts == _TRUE_){
  ...
  if (ptsz->integrate_wrt_m200m == 1 && ptsz->has_200c == 1)
   ...
   ptsz->index_integrate_wrt_mvir = ptsz->index_density_profile + 1;
   ptsz->index_integrate_wrt_m500c = ptsz->index_integrate_wrt_mvir + 1;
   ptsz->index_integrate_wrt_m200m = ptsz->index_integrate_wrt_m500c + 1;
...
for (index_l=0;index_l<ptsz->nlSZ;index_l++){
 ...
 // printf("counting integrands\n");
   ...
   ptsz->index_integrand_id_kSZ_kSZ_gal_2h_first = ptsz->index_integrand_id_kSZ_kSZ_gal_1h_fft_last + 1;
...
us = get_truncated_nfw_profile_at_z_m_k_xout(z,m,k,xout,delta,pba,ptsz);
// printf("us %.3e\n",us);
double ug_at_ell;
...
us = evaluate_truncated_nfw_profile(xout,pvectsz,pba,ptsz,0);
   ...
   // double m_asked = pvectsz[ptsz->index_m200c]; // in Msun/h
   // double z_asked = pvectsz[ptsz->index_z];
   // us = get_density_profile_at_l_M_z(l_asked,m_asked,z_asked,ptsz);
...
double rho_crit = (3./(8.*_PI_*_G_*_M_sun_))
                                ...
                                *pvecback[pba->index_bg_rho_crit]
                                /pow(pba->h,2);
// double rho_crit = 1.;
// printf("z = %.5e rho_cit = %.5e\n",z,pvecback[pba->index_bg_rho_crit]);
r_delta = pow(3.*m/(4.*_PI_*delta*rho_crit),1./3.);
c_delta = get_c200m_at_m_and_z_D08(m,z);
...
int evaluate_c200c_D08(double * pvecback,
                        ...
                        struct tszspectrum * ptsz)
{
double M = pvectsz[ptsz->index_m200c];// mass in  Msun/h
double A = 5.71;
double B = -0.084;
double C = -0.47;
double M_pivot = 2.e12; // pivot mass in Msun/h
...
pvectsz[ptsz->index_c200c] = c200c;
}
