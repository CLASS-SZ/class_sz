...
int fourier_pk_at_k_and_z(
  ...
  else if ((k < 0.) || (k > exp(pfo->ln_k[pfo->k_size-1]))){
    *out_pk = 0.;
...
int fourier_init(
  ...
  // pfo->ln_k_size_for_tSZ = (int)(log(pfo->k_max_for_pk_in_tSZ
  //                                    /pfo->k_min_for_pk_in_tSZ)
  //                                /log(10.)*pfo->k_per_decade_for_tSZ) + 2;
  //
  // class_alloc(pfo->ln_k_for_tSZ,pfo->ln_k_size_for_tSZ*sizeof(double),pfo->error_message);
  // int i;
  // for (i=0; i<pfo->ln_k_size_for_tSZ; i++)
  //     pfo->ln_k_for_tSZ[i]=log(pfo->k_min_for_pk_in_tSZ)+i*log(10.)/pfo->k_per_decade_for_tSZ;
...
int fourier_indices(
  ...
  //pfo->has_pk_cb = _TRUE_; //BB: added for class_sz
