...
def run(args):
    ...
    ax.plot(Masses,100.*(dndm/(nm/np.log(10)/cosmology['h']**3/Masses)-1.), label='class_sz/ccl in percents')
    ax.legend()
    ...
    ax.plot(Masses,dndm, label='class_sz')
    ax.plot(Masses,(nm/np.log(10)/cosmology['h']**3/Masses), label='ccl',ls='--')
    ax.legend()
